...
const formState = reactive<PostModel.PostUpdateDto>({
  categoryIds: [], content: "", cover: "", description: "", id: 0, title: ""});
const options = ref<CategoryModel.CategorySimpleVo[]>([])
interface IProps {
  visible: boolean;
  data: PostModel.PostVo;
}
...
watch(() => props.data, (val) => {
  ...
  formState.categoryIds = val?.categories?.map(category => {return category.id}) || []
}, {immediate: true})
...
const handleOk = () => {
  formRef.value?.validateFields().then(async () => {
    const res = await updatePost(formState as PostModel.PostUpdateDto)
    if (res.code === 200) {
...
<template>
  <a-modal v-model:visible="dialogVisible" width="100%" wrap-class-name="full-modal" title="帖子更新" @cancel="handleCancel">
