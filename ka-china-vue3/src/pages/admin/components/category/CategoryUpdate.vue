...
const formState = reactive<CategoryModel.CategoryUpdateDto>({id: 0, name: ""});

interface IProps {
    visible: boolean;
    data: CategoryModel.CategoryVo;
}
...
const handleOk = () => {
    formRef.value?.validateFields().then(async () => {
        const res = await updateCategory(formState as CategoryModel.CategoryUpdateDto)
        if (res.code === 200) {
...
<template>
    <a-modal v-model:visible="dialogVisible" title="分类更新" @cancel="handleCancel">
